...
The width of the two components (section and aside) is making them overflow of the existing container. Having a css attibute box-sizing: border-box makes sure the CSS box model wraps everything with the given width
This may align the aside(red module box) besides the section (green module box) but on changing the screen width the heights of red and green boxes would not always align. A better way is to wrap these 2 modules(red and green) in one div container having css attribute set to flex and flex-flow: row
